...
class MainSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        if "francetvinfo" in urlsite :
            title = response.css('#js--content > h1').extract()
            description = response.css("#js--content > div.c-body").extract()
            devimages = response.css("#js--content > div.c-cover > div > figure > picture > img").extract()
